		...
		private void Export (string nodeId, string labelToNext, Dictionary<string, List<InternalAssetData>> groupedSources, Action<string, string, Dictionary<string, List<InternalAssetData>>, List<string>> Output, bool isRun) {
			...
			foreach (var groupKey in groupedSources.Keys) {
				...
				foreach (var source in inputSources) {
					...
					if (destinationSourcePath.StartsWith(AssetGraphSettings.BUNDLEBUILDER_CACHE_PLACE)) {
						var depth = AssetGraphSettings.BUNDLEBUILDER_CACHE_PLACE.Split(AssetGraphSettings.UNITY_FOLDER_SEPARATOR).Length + 1;
						
						var splitted = destinationSourcePath.Split(AssetGraphSettings.UNITY_FOLDER_SEPARATOR);
						var reducedArray = new string[splitted.Length - depth];
						
						Array.Copy(splitted, depth, reducedArray, 0, reducedArray.Length);
						var fromDepthToEnd = string.Join(AssetGraphSettings.UNITY_FOLDER_SEPARATOR.ToString(), reducedArray);
