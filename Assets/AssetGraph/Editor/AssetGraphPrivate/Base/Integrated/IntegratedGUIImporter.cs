...
namespace AssetGraph {
	public class IntegratedGUIImporter : INodeBase {
		...
		public void Run (string nodeId, string labelToNext, string package, Dictionary<string, List<InternalAssetData>> groupedSources, List<string> alreadyCached, Action<string, string, Dictionary<string, List<InternalAssetData>>, List<string>> Output) {
			...
			var samplingDirectoryPath = FileController.PathCombine(AssetGraphSettings.IMPORTER_SAMPLING_PLACE, nodeId, GraphStackController.PlatformFolder(package));
			var outputDict = new Dictionary<string, List<InternalAssetData>>();

			var nodeDirectoryPath = FileController.PathCombine(AssetGraphSettings.IMPORTER_CACHE_PLACE, nodeId, GraphStackController.PlatformFolder(package));
